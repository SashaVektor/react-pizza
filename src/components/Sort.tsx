...
import { SortPropEnum, SortState } from "../redux/slices/filter/types";
    ...
    <div className="sort" ref={sortRef}>
      ...
      {isVisible && <div className="sort__popup">
        <ul>
          {
            list.map((obj, i) => (
              <li key={i} onClick={() => onClickListItem(obj)} className={value.sortProp === obj.sortProp ? 'active' : ''}>{obj.name}</li>))
          }
        </ul>
      </div>}
    </div>
